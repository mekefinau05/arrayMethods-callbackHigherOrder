...
// CODE HERE
const evenNumbers  = mixedNumbers.filter((element, index, array) => {
  if(array[index] % 2 === 0) {
...
// CODE HERE
const postTaxPrices = prices.map((element, index, array) => {

  array[index] = element * 1.07
  return array[index]
});
...
// CODE HERE
const totalPopulation  = populations.reduce((runningTotal, curElement, curIndex, wholeArray) => {

  wholeArray[curIndex] = curElement
...
const orderTotals = orders.map((cost) => {
  return cost.price * (cost.tax + 1)
})
...
//const bobsTotal = purchases.filter((person) => person.owner = "Bob").reduce((runningTotal, currPrice) => { 

// return runningTotal = runningTotal + currPrice.price
// }, 0)

//console.log(person.owner)

const bobsTotal = purchases.filter((person) => {
  return person.owner === "Bob" 
